...
- (void)initializer {
    ...
    _colorIndicatorView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, self.bounds.size.width - 50, self.bounds.size.height)];
...
- (void)moveWithoutGestureTranslation:(CGFloat)translation {
    CGPoint center = {self.contentView.center.x + translation, self.contentView.center.y};
    [self.contentView setCenter:center];
    [self animateWithOffset:CGRectGetMinX(self.contentView.frame)];
